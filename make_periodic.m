function [blk] = make_periodic(blk,next_block,next_patch,nbnow,pitch)
...
if(im_next_block~=0)
    ...
    if(im_next_patch==2)
        ...
        if( (blk{ib}.y(1,1) - blk{im_next_block}.y(end,1)) < -0.1*pitch ); yoffset =-pitch; end
        xup = blk{im_next_block}.x(end,:);
        yup = blk{im_next_block}.y(end,:)+yoffset;
...
if(ip_next_block~=0)
    ...
    if(ip_next_patch==1)
        ...
        if( (blk{ib}.y(end,1) - blk{ip_next_block}.y(1,1)) < -0.1*pitch ); yoffset =-pitch; end
        xdn = blk{ip_next_block}.x(1,:);
        ydn = blk{ip_next_block}.y(1,:)+yoffset;


        blk{ib}.x(end,:) = (xdn + blk{ib}.x(end,:))*0.5;
        blk{ib}.y(end,:) = (ydn + blk{ib}.y(end,:))*0.5;
    ...
    if(ip_next_patch==2)
        ...
        if( (blk{ib}.y(end,1) - blk{ip_next_block}.y(end,1)) < -0.1*pitch ); yoffset =-pitch; end
        xdn = blk{ip_next_block}.x(end,:);
        ydn = blk{ip_next_block}.y(end,:)+yoffset;
...
if(jm_next_block~=0)
    ...
    if(jm_next_patch==2)
        ...
        if( (blk{ib}.y(1,1)' - blk{jm_next_block}.y(end,1)) < -0.1*pitch ); yoffset =-pitch; end
        xup = blk{jm_next_block}.x(end,:)';
        yup = blk{jm_next_block}.y(end,:)'+yoffset;
    ...
    if(jm_next_patch==3)
        yoffset = 0.0;
        if( (blk{ib}.y(1,1) - blk{jm_next_block}.y(1,1)) >  0.1*pitch ); yoffset = pitch; end
        if( (blk{ib}.y(1,1) - blk{jm_next_block}.y(1,1)) < -0.1*pitch ); yoffset = -1*pitch; end
        yoffset=0;
        xup = blk{jm_next_block}.x(:,1);
        yup = blk{jm_next_block}.y(:,1)+yoffset;
...
if(jp_next_block~=0)
    ...
    if(jp_next_patch==1)
        ...
        if( (blk{ib}.y(1,end)' - blk{jp_next_block}.y(1,1)) < -0.1*pitch ); yoffset =-pitch; end
        xdn = blk{jp_next_block}.x(1,:)';
        ydn = blk{jp_next_block}.y(1,:)'+yoffset;
    ...
    if(jp_next_patch==2)
        ...
        if( (blk{ib}.y(1,end)' - blk{jp_next_block}.y(end,1)) < -0.1*pitch ); yoffset =-pitch; end
        xdn = blk{jp_next_block}.x(end,:)';
        ydn = blk{jp_next_block}.y(end,:)'+yoffset;
