...
have_title=~isempty(find(strcmp(tdatanames,'title')==1, 1));  %true if have title
...
have_Nvar=~isempty(find(strcmp(tdatanames,'Nvar')==1, 1));  %check if have 'Nvar'

   % Number of variables
if(~have_Nvar)  %check if number of variables given
                              %if not given then calculate
                              %from tdata.varnames
    have_varnames=~isempty(find(strcmp(tdatanames,'varnames')==1, 1));
    if(have_varnames)
